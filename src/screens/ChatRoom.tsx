  ...
  const handleCheck = async (organization: IOrganization) => {
    ...
    const handleDeleteMessage = async (message: any) => {
      ...
      if (!doc.exists) {
        ...
        organizationRef.collection('messages').doc(message._id).update({
          isDeleted: true
        })
    }
   ...
   <ImageBackground
      ...
      <Block paddingHorizontal={sizes.s}>
          ...
          renderMessageText={(props) => {
            ...
            return (
              <Block card flex={0} black={isMine} white={!isMine}>
                <Text white={isMine}>{props?.currentMessage?.isDeleted ? 'This message has been deleted.' : props?.currentMessage?.text}</Text>
        ...
        onLongPress={(context, message) => {
          if(message?.user?._id === user._id && !message?.system && !message?.isDeleted) {
            Alert.alert(
              'Delete message',
              ...
              [
                ...
                { text: 'Copy', onPress: () => setMessage(message?.text) },
