...
const Posts = ({ route,  navigation }) => {
    ...
    const handlePress = (type: String, _id: String) => {
                        ...
                        text: "OK", onPress: () => {
                            (async () => {
                                ...
                                .then((res) => {
                                    ...
                                    selectedPost[index]?.images?.forEach((item) => {
        ...
        if(Platform.OS === 'android' && type !== 'delete') {
            ToastAndroid.showWithGravityAndOffset(
                type === 'upvote' ? 'Upvoted' : type === 'downvote' ? 'Downvoted' : type === 'favorite' ? 'Added to favorites' : type === 'unfavorite' ? 'Removed from favorites' : '',
                ToastAndroid.SHORT,
                ToastAndroid.BOTTOM,
                25,
                50
